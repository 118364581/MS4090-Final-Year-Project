...
namespace MS4090_FYP_118364581_Conor_McMahon
{
    internal class JordanNN
    {
        ...
        private BasicNetwork jordanNetwork;

        public JordanNN(string CSVPath, int window_size, double trainpercent, int neurons, bool Validation)
        {
            ...
            var jordanNetwork = (BasicNetwork)CreateJordanNetwork(trainingSet.InputSize, neurons);
            this.jordanNetwork = jordanNetwork;

        }

        private IMLMethod CreateJordanNetwork(int input, int neurons)
        {
        ...
        public double[] Train()
        {
            ...
            IMLTrain trainMain = new ResilientPropagation(jordanNetwork, trainingSet);
            

            int epoch = 0; double[] Train_errors = new double[5000];
            while (epoch < 5000)  //&& !stop.ShouldStop()
            {
                trainMain.Iteration();
                //Console.WriteLine("Training Jordan: " + " Epoch #" + epoch + " Error:" + trainMain.Error);
        ...
        public double[] Test()
        {
            ...
            double[] ideal_midpoint = new double[test_size];

            ideal_midpoint = Midpoint(NNTest_Output);
            foreach (IMLDataPair pair in testSet)
            {
                IMLData output = jordanNetwork.Compute(pair.Input);
                //Error[count] = (Math.Pow(output[0] - pair.Ideal[0], 2) + Math.Pow(output[1] - pair.Ideal[1], 2) + Math.Pow(output[2] - pair.Ideal[2], 2) + Math.Pow(output[3] - pair.Ideal[3], 2)) / 4.0;
                Error[count] = (Math.Pow(ideal_midpoint[count] - 1.25, 2) * Math.Pow(output[0] - pair.Ideal[0], 2) + Math.Pow(ideal_midpoint[count] - 0.4, 2) * Math.Pow(output[1] - pair.Ideal[1], 2) + Math.Pow(ideal_midpoint[count] - 0.0, 2) * Math.Pow(output[2] - pair.Ideal[2], 2) + Math.Pow(ideal_midpoint[count] + 1.3, 2) * Math.Pow(output[3] - pair.Ideal[3], 2)) / 4.0;
